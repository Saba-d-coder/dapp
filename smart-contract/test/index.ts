...
describe(CollectionConfig.contractName, function () {
  ...
  it('Before any sale', async function () {
    ...
    await expect(contract.connect(whitelistedUser).mint(1, {value: getPrice(SaleType.WHITELIST, 1)})).to.be.revertedWith('The sale is paused!');
    ...
    await expect(contract.connect(holder).mint(1, {value: getPrice(SaleType.WHITELIST, 1)})).to.be.revertedWith('The sale is paused!');
    ...
    await expect(contract.connect(owner).mint(1, {value: getPrice(SaleType.WHITELIST, 1)})).to.be.revertedWith('The sale is paused!');
